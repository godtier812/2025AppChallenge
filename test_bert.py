...
clinical_bert_model = AutoModel.from_pretrained(model_name, use_safetensors=False)
...
def classify_disease_and_severity(text):
    ...
    # Use the pooled output (embedding for classification)
    pooled_output = outputs.pooler_output  # shape: (1, hidden_size)

    # Dummy classification logic based on pooled output mean value
    # You should replace this with a proper classifier model
    severity_score = pooled_output.mean().item()

    print(f"Bert model response: {severity_score}")  # Debugging line

    # Modify for more advanced classes if necessary (Assuming binary classification: 0: disease, 1: severity level)
    severity_label = "Mild" if severity_score == 0 else "Severe"
...
if __name__ == '__main__':
    ...
    #setup varialble imagepath with image path of lab report
    imagepath = "/Users/ashrithrepaka/Documents/Ashrith/Coding/2025AppChallenge/reports/tb.png"

    #use function extract_text_from_image by passing imagepath - extract text to extracted_text
    ...
    extracted_text_from_image = pytesseract.image_to_string(gray)
    print(f"Extracted Text from Image: {extracted_text_from_image}")
    #call analyze_with_clinicalBert with extracted text and print the result
    result = analyze_with_clinicalBert(extracted_text_from_image)
    print(result)
